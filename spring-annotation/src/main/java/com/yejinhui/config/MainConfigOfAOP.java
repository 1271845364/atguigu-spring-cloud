...
import com.yejinhui.aop.LogAspects;
import com.yejinhui.aop.MathCalculator;

/**
 * AOP：[动态代理] 在程序运行期间动态的将某段代码切入到指定方法指定位置进行运行的编程方式
 * 
 * 1、导入aop模块：Spring AOP:spring-aspects
 * 2、定义一个业务逻辑类（MathCalculator）：在业务逻辑运行的时候将日志进行打印（方法之前、方法运行结束、方法运行出现异常、xxx）
 * 3、定义一个日志切面类（LogAspects）:切面类里面的方法需要动态感知MathCalcularor.div运行到哪里
 * 前置通知(@Before)：logStart：在目标方法(div)运行之前运行
 * 后置通知(@After)：logEnd：在目标方法(div)运行结束后运行（无论方法正常结束还是异常结束）
 * 返回通知(@AfterReturning)：logReturn：在目标方法(div)正常返回之后运行
 * 异常通知(@AfterThrowing)：logException：在目标方法(div)出现异常以后运行
 * 环绕通知(@Around)：动态代理，手动推动目标方法运行（joinPoint.procced()） 4、给切面类的目标方法标注何时何地运行（通知注解）
 * 5、将切面类和业务逻辑（目标方法所在类）都加入到容器中
 * 6、必须告诉spring那个类是切面类(给切面类上加一个注解 @Aspect )
 * 7、给配置类中加上 @EnableAspectJAutoProxy [开启基于注解的aop模式]
 * 		在Spring中有很多的 @EnableXXX
 * 
 * 三步：
 * 1）、将业务逻辑组件和切面类都加入到容器中；告诉spring容器那个是切面类（ @Aspect ）
 * 2）、在切面类上的每一个通知方法上标注通知注解，告诉spring何时何地运行（切入点表达式）
 * 3）、开启基于注解的aop模式   @EnableAspectJAutoProxy
 * 
 * AOP原理[看给容器注册了什么组件、这个组件什么时候工作、这个组件有什么功能]：
 ...
 * 			1）、先获取ioc容器已经定义了的需要创建对象的所有的 BeanPostProcessor
 * 			2）、给容器加别的 BeanPostProcessor
 * 			3）、优先注册实现了 PriorityOrdered 这个接口的 BeanPostProcessor
 ...
 *				1）、先从缓存中获取当前bean，如果能获取到，说明bean是之前被创建的，直接使用；否则在创建
 *					只要创建好的bean都会被缓存起来
 ...
 *							}
 * 					2）、resolveBeforeInstantiation(beanName, mbdToUse);真正的去创建一个bean实例，和3.6流程一样
 *					3）、
 *
 ...
 * 			         判断每一个增强器是否是AbstractAspectJAdvice类型的；返回true
 * 			2）、永远返回false
 *  2）、创建对象
 *  	postProcessAfterInstantiation
 *  		return wrapIfNecessary(bean,beanName,cacheKey);//包装如果需要的情况下
 *  		a）、获取当前bean的所有增强器（通知方法）	Object[] specificInterceptors
 *  			1）、找到候选的增强器（找那些通知方法是需要切入当前bean方法的）
 *  			2）、获取到能在bean使用的增强器
 *  			3）、给增强器排序
 ...
 *  			b）、遍历所有的增强器，将其转换为Interceptor
 *  				registry.getInterceptors(advisor);
 *  			c）、将增强器转换为List<MethodInterceptor>
 *  				如果是MethodInterceptor，直接加入到集合中
 *  				如果不是，使用AdvisorAdapter将增强转换器转为MethodInterceptor；
...
@EnableAspectJAutoProxy
@Configuration
@ComponentScan(value = { "com.yejinhui.aop" })
