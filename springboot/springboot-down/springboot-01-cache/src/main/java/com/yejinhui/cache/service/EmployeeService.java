...
public class EmployeeService {
     ...
     * @param id
     * @return
     * @Cacheable的几个属性： cacheNames/value：指定缓存组件的名字；将方法的返回结果放在那个缓存中，是数组的方式，可以指定多个缓存
     * key：缓存数据使用的key；可以使用它来指定。默认是使用方法参数的值 1-方法的返回值
     * 编写SpEL； #id；参数id的值  等同于 #a0 #p0 #root.args[0]，0代表参数索引
     ...
     * org.springframework.boot.autoconfigure.cache.SimpleCacheConfiguration【默认的】
     ...
     * 如果缓存中没有就运行方法并将结果放入缓存；以后再来调用就可以直接调用缓存中的数据
     * <p>
     * 核心：
     * 1）、使用CacheManager【ConcurrentMapCacheManager】按照名字得到Cache【ConcurrentMapCache】组件
     * 2）、key是使用keyGenerator生成的，默认使用的是SimpleKeyGenerator
     */
    @Cacheable(cacheNames = {"emp"}/*,key = "#root.methodName+'['+#id+']'",keyGenerator = "myKeyGenerator",condition = "#a0>1",unless = "#a0 == 2"*/)
     ...
     * 场景：更新数据库中的表的数据，同时更新缓存
     * 运行时机：
     * 1、先调用目标方法
     * 2、将目标方法的结果缓存起来
     * <p>
     * 测试步骤：
     * 1、查询1号员工；查询到的结果会放到缓存中
     * key：1  value：lastName：张三
     * 2、以后查询还是之前的结果
     * 3、更新1号员工；【lastName=zhangsan；gender=0】
     * 将方法的返回值放入到缓存中
     * key：传入的employee对象 值：缓存的employee对象
     * 4、查询1号员工？
     * 应该是更新后的员工；
     * key = "#employee.id"：使用传入参数的员工id
     * key = "#result.id"：使用返回后的id
     * @Cacheable 的key不能使用#result 原因：因为@Cacheable 是先走缓存，先通过key获取缓存的
